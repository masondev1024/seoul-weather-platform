...
forecast_candidates as (
    ...
    inner join forecast_scope
       ...
       and forecast_scope.issued_at between expected_population.valid_at - interval '1' hour * expected_population.lower_hours_before_valid
                                      and expected_population.valid_at - interval '1' hour * expected_population.upper_hours_before_valid
    where forecast_scope.collected_at <= cast({{ evaluation_as_of }} as timestamp(6))
      and forecast_scope.manifest_event_at_utc <= cast({{ evaluation_as_of }} as timestamp(6))
),
