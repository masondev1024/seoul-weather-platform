...
expected_slots as (
    ...
    cross join unnest(
        case
            when global_forecast_horizon_at is null
                then cast(array[] as array(timestamp(6)))
            else sequence(
                snapshot_as_of_hour,
                global_forecast_horizon_at,
                interval '1' hour
            )
        end
    ) as slot(slot_at)
...
complete_prefix as (
    ...
    where slot_matrix.slot_at is not null
      ...
      and (
          place_rollup.first_incomplete_at is null
          or slot_matrix.slot_at < place_rollup.first_incomplete_at
      )
    group by 1
)
...
select
    ...
    case
        when place_rollup.expected_forecast_hour_count > 0
         ...
         and place_rollup.first_incomplete_at is null
            then 'complete'
        else 'incomplete'
    end as availability_status,
    population_revision.source_population_revision
from population
cross join horizon
