...
class QualityEvaluationWindow:
    ...
    def as_payload(self) -> dict[str, object]:
        return {
            ...
            "forecast_load_start_date": self.forecast_load_start_date.isoformat(),
            "forecast_load_end_date": self.forecast_load_end_date.isoformat(),
            "dbt_vars": self.as_dbt_vars(),
        }
...
def window_payload(window: QualityEvaluationWindow) -> dict[str, object]:
    return window.as_payload()


def window_from_payload(payload: Mapping[str, Any]) -> QualityEvaluationWindow:
    if not isinstance(payload, Mapping):
        raise QualityWindowError("weather quality window payload is invalid")
    try:
        window = QualityEvaluationWindow(
            evaluation_run_id=_require_safe_run_id(str(payload["evaluation_run_id"])),
            evaluation_as_of=_parse_aware_datetime(payload["evaluation_as_of"]),
            window_start_date=_parse_payload_date(payload["window_start_date"]),
            window_end_date=_parse_payload_date(payload["window_end_date"]),
            forecast_load_start_date=_parse_payload_date(
                payload["forecast_load_start_date"]
            ),
            forecast_load_end_date=_parse_payload_date(payload["forecast_load_end_date"]),
        )
    except KeyError as exc:
        raise QualityWindowError("weather quality window payload is incomplete") from exc
    if payload.get("dbt_vars") != window.as_dbt_vars():
        raise QualityWindowError("weather quality window payload dbt vars are unstable")
    return window
...
def _parse_payload_date(value: Any) -> date:
    if not isinstance(value, str) or not _ISO_KST_DATE_RE.fullmatch(value):
        raise QualityWindowError("weather quality window payload date is invalid")
    try:
        return date.fromisoformat(value)
    except ValueError as exc:
        raise QualityWindowError("weather quality window payload date is invalid") from exc


def _parse_aware_datetime(value: Any) -> datetime:
    if not isinstance(value, str):
        raise QualityWindowError("weather quality window payload timestamp is invalid")
    try:
        parsed = datetime.fromisoformat(value.replace("Z", "+00:00"))
    except ValueError as exc:
        raise QualityWindowError(
            "weather quality window payload timestamp is invalid"
        ) from exc
    if parsed.tzinfo is None or parsed.utcoffset() is None:
        raise QualityWindowError(
            "weather quality window payload timestamp must be timezone-aware"
        )
