"""Manual one-date internal Weather forecast-quality Gold backfill DAG."""
...
DEFAULT_PARAMS = {
    ...
    "backfill_date": Param(
        default="",
        type="string",
        description="Exactly one complete past KST date in YYYY-MM-DD form.",
    ),
    "confirmation": Param(
        default="",
        type="string",
        description=f"Must equal {QUALITY_BACKFILL_CONFIRMATION}.",
    ),
}
...
def resolve_forecast_quality_window(**context) -> dict[str, object]:
    ...
    window = resolve_backfill_quality_window(
        backfill_date=params.get("backfill_date"),
...
with DAG(
    dag_id="weather_forecast_quality_backfill",
    description="Manual internal Weather forecast-quality Gold backfill for one KST date.",
    ...
    tags=["ask_seoul", "weather", "quality", "gold", "dbt", "backfill", "internal"],
) as dag:
