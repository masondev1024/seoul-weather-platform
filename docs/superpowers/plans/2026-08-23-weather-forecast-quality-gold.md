# Weather Forecast-Quality Gold Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a daily, replayable, R2/Iceberg-only Weather forecast-quality pipeline that compares short-range D-1/D-2/D-3 temperature and precipitation forecasts with complete hourly observations without changing D1, Worker, or the existing serving path.

**Architecture:** A pure-Python runtime contract freezes one seven-day KST evaluation window. dbt reads bounded publishable Forecast Bronze and partitioned Observation Bronze into dedicated quality Silver models, produces run-versioned grid/hour/day Gold histories, and exposes only dates whose run is present in an immutable success manifest. Two inert-by-default Airflow DAGs own daily and one-date backfill orchestration through the existing one-slot Weather Trino lane.

**Tech Stack:** Python 3.11, Airflow 3.2.2, dbt-core 1.10.22, dbt-trino 1.10.2, Trino 482, Iceberg v2, Cloudflare R2 Data Catalog, pytest, JSON Schema.

**Spec:** `docs/superpowers/specs/2026-08-22-weather-forecast-quality-gold-design.md`
