...
- protected mode의 native protection API readback과 모든 mode의 최초 cutover 명시 승인 전에는 production runner 등록·활성화와 `main` 자동 배포를 금지한다. `guarded_private`는 private 단일 소유자 사고 방지 경계이며, 승인 뒤에도 exact same-repository merge 증거를 재검증한다. public/internal 전환 또는 추가 writer면 guarded 배포를 중단한다.
...
Every external `uses:` must match `^[^@]+@[0-9a-f]{40}$`; local actions are forbidden. Self-hosted policy is event-aware: PR와 `workflow_dispatch`는 금지하고, CI runtime은 `protected` `push`의 exact `refs/heads/dev|main`만 허용한다. 자동배포는 `.github/workflows/deploy-main.yml` 하나에서만 허용한다. trigger는 `workflow_run`의 exact `CI`/`completed`/`main`, guard는 `guarded_private` 또는 `protected`와 `enabled`, source workflow name `CI`, suffix 없는 path `.github/workflows/ci.yml`, 별도 `head_branch=main`, event/status/conclusion과 `github.workflow_sha == workflow_run.head_sha`를 모두 요구한다. guarded는 private exact same-repository merged PR을 다시 검증하고 public/internal 또는 추가 writer에서 중단하며, protected는 native protection을 다시 검증한다. GitHub-hosted `verify-main` 성공 뒤 `[self-hosted, windows, weather-prod]`의 `deploy-main`만 실행할 수 있다. `repository_dispatch`, Release, manual dispatch 또는 다른 workflow path가 self-hosted deploy에 도달하면 finding이다. `docker compose exec`와 `run`은 내부 argv와 무관하게 mutation으로 판정하며 direct `docker compose config`와 `ps`만 기존 read-only 판정을 유지한다.
...
Self-hosted workflow/job에는 `defaults.run`을 두지 않고 job `container`와 `services`를 금지한다. `working-directory`를 지정할 수 없고, Deploy Main run step의 shell은 exact `pwsh`다. workflow/job `env`는 `PATH`, `PYTHONPATH`, `PYTHONHOME`, `PIP_*`, `PSModulePath`, `LD_*`, `NODE_*`, `WEATHER_DEPLOY_TARGET_PATH`처럼 executable/import/target을 바꾸는 이름을 정의할 수 없다. Deploy Main에는 package-install run step이 없어야 한다. guarded CLI step은 `GH_TOKEN`에 workflow token을, protected CLI step은 `GH_TOKEN`에 `WEATHER_GOVERNANCE_READ_TOKEN`을 사용하며 두 mode 모두 exact `GOVERNANCE_MODE`와 `DEPLOYMENT_ENABLED`를 전달한다. protected secret은 `Administration`, `Actions`, `Checks`, `Contents`, `Pull requests` read만 가진다.
...
- `guarded_private`은 private 단일 소유자 사고 방지 경계다. 별도 최초 cutover 승인과 exact merge 재검증 전에는 production runner 또는 `Deploy Main` 활성화로 넘어갈 수 없고, public/internal 또는 추가 writer면 guarded 배포를 중단한다. protected readback이 성공해도 runner 설치·Airflow 상태 변경은 최초 cutover의 별도 승인 전까지 수행하지 않는다.
